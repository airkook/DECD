...
public class DefaultBaseDao<T, PK extends Serializable> extends HibernateDaoSupport implements IBaseDao<T, PK> {
     ...
     * 跟据对象新增数据
     * 
     * @param newInstance 新增对象
     * @throws BaseDaoException DAO的专用Exception3
     */
     ...
     * @param id 预删除数据的ID字段
     * @return 是否存在数据并删除成功
     * @throws BaseDaoException DAO的专用Exception
     */
    ...
    public PageResults<T> findAll(int pageNo, int pageSize) throws BaseDaoException {
    ...
    public PageResults<T> findAll(List<PK> ids, int pageNo, int pageSize) throws BaseDaoException {
     ...
     * @return
     */
    protected int findCountByCriteriaUsingCountDistinct(final DetachedCriteria detachedCriteria) {
        Integer count = (Integer) getHibernateTemplate().execute(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException {
                Criteria criteria = detachedCriteria.getExecutableCriteria(session);
     ...
     * @return
     */
    public PageResults findPageByCriteria(final DetachedCriteria detachedCriteria, final int pageSize, final int pageNo) {
     ...
     * @return
     */
    protected PageResults findPageByCriteria(final DetachedCriteria detachedCriteria, final int total,
    ...
    protected PageResults findPageByCriteria(final DetachedCriteria detachedCriteria, final int total,
        ...
        return (PageResults) getHibernateTemplate().execute(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException {
                int totalCount = 0;
                ...
                if (total < 0) {
                    ...
                    totalCount = (totalCountObject == null) ? 0 : totalCountObject.intValue();
                }
                else {
                    totalCount = total;
                }
                ...
                if (totalCount == 0) {
                    ...
                    if (orderBy != null) {
                        ...
                        ids = criteria.setProjection(
                                Projections.projectionList().add(Projections.distinct(Projections.id())).add(
                                        Projections.property(orderStr))).addOrder(orderBy).setFirstResult(
                                retValue.getResultsFrom()).setMaxResults(retValue.getPageSize()).list();
                        ...
                        criteria.setResultTransformer(CriteriaSpecification.DISTINCT_ROOT_ENTITY).add(
                                Restrictions.in("id", ids));
                        items = criteria.setFirstResult(0).setMaxResults(Integer.MAX_VALUE).list();
                    }
                    else {
                        // 不排序的实现
                        criteria.setProjection(projection);
                        criteria.setResultTransformer(CriteriaSpecification.DISTINCT_ROOT_ENTITY);
                        items = criteria.setFirstResult(retValue.getResultsFrom())
                                .setMaxResults(retValue.getPageSize()).list();
                    }
     ...
     * @return
     */
    public PageResults findPageByCriteria(final DetachedCriteria detachedCriteria, int pageSize, int pageNo,
            final Order orderBy) {
        return findPageByCriteria(detachedCriteria, -1, pageSize, pageNo, orderBy);
    }

    public PageResults findPageByHsql(String hsql, Map paraMap, int pageSize, int pageNo) throws BaseDaoException {
        PageResults retValue = new PageResults();
        if (hsql != null) {
            Session session = getSession();
            org.hibernate.Query query = session.createQuery(hsql);
            ...
            if (retValue.getTotalCount() == 0) {
                ...
                retValue.setResults(query.setFirstResult(retValue.getResultsFrom()).setMaxResults(pageSize).list());
            }
            releaseSession(session);
        }
        return retValue;
    ...
    public PageResults findPageByHsql(Class entityClass, Map paramMap, Map orderMap, int pageSize, int pageNo)
    ...
    public PageResults findPageBySql(String sql, Map paraMap, int pageSize, int pageNo) throws BaseDaoException {
        PageResults retValue = new PageResults();
        if (sql != null) {
            Session session = getSession();
            org.hibernate.Query query = session.createSQLQuery(sql);
     ...
     */
    protected <ID> boolean insertIdsInRangToDetachedCriteria(DetachedCriteria dCriteria, String inProperty, List<ID> ids) {
        if (ids != null) {
            if (ids.size() == 0) {
                return true;
            }
    ...
    public Integer findBySql(String sql) {
        Session session = getSession();
        org.hibernate.Query query = session.createSQLQuery(sql);
    ...
    public void InitDBTypeName() {
        try {

            String dbName = getSession().connection().getMetaData().getDatabaseProductName();
            System.out.println("数据库类型为：" + dbName);
    ...
    public void saveOrUpdateAll(List<T> entities) throws BaseDaoException {
        this.getHibernateTemplate().saveOrUpdateAll(entities);
    }

    public List findBySql_toMap(String sql) {
        List lst = null;
