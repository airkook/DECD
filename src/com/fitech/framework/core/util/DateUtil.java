...
public class DateUtil {
    ...
    public static final int[] MONTH_ARRAY = { 0, Calendar.JANUARY, Calendar.FEBRUARY, Calendar.MARCH, Calendar.APRIL,
     ...
     * 用于格式化日期
     */
    public static final SimpleDateFormat PERIOD_DATE_FORMAT = new SimpleDateFormat("dd-MM-yyyy");
     ...
     * 周频度
     */
    public static final int FREQ_WEEK = 6;

    /**
     * 实时频度
     */
    // public static final int FREQ_CURRTIME = 0;
    ...
    public static String getPeriodTodayDateStr() {

        Calendar calendar = Calendar.getInstance();

        return (PERIOD_DATE_FORMAT.format(calendar.getTime()));
    }
    ...
    public static List getDateList(String startDate, String endDate, Integer freq) {
        ...
        while (startCalendar.before(endCalendar) || startCalendar.equals(endCalendar)) {
            if (freq.intValue() == DateUtil.FREQ_MONTH && DateUtil.isMonthDay(newDateStr)) {
            ...
            /* 季频度 */
            else if (freq.intValue() == DateUtil.FREQ_SEASON && DateUtil.isSeasonDay(newDateStr)) {
                dateList.add(newDateStr);
            ...
            /* 半年频度 */
            else if (freq.intValue() == DateUtil.FREQ_HALFYEAR && DateUtil.isHalfYearDay(newDateStr)) {
                dateList.add(newDateStr);
            ...
            /* 年频度 */
            else if (freq.intValue() == DateUtil.FREQ_YEAR && DateUtil.isYearDay(newDateStr)) {
                dateList.add(newDateStr);
    ...
    public static boolean isSeasonDay(String date) {
        if (date == null || date.equals(""))
            return false;
        return date.endsWith("03-31") || date.endsWith("06-30") || date.endsWith("09-30") || date.endsWith("12-31");
    }
    ...
    public static boolean isCustomSeason(String date) {
        if (date == null || date.equals(""))
            return false;
        return date.endsWith("03-20") || date.endsWith("06-20") || date.endsWith("09-20") || date.endsWith("12-20");
    }
     ...
     * @return
     */
    public static List getYearMonthInterval(String _startDate, String _endDate, int freq) {
        List result = null;
        String startDate = _startDate.split("-")[0] + "-" + _startDate.split("-")[1] + "-01";
        String endDate = null;
        int mon = Integer.parseInt(_endDate.split("-")[1]);
        try {

            if (mon == 1 || mon == 3 || mon == 5 || mon == 7 || mon == 8 || mon == 10 || mon == 11 || mon == 12)
                endDate = _endDate.split("-")[0] + "-" + _endDate.split("-")[1] + "-31";
            else
                endDate = _endDate.split("-")[0] + "-" + _endDate.split("-")[1] + "-30";
    ...
    public static boolean checkYearMonth(String yearStr, String monthStr) {
        ...
        try {
            if (yearStr == null || monthStr == null || yearStr.trim().equals("") || monthStr.trim().equals(""))
     ...
     * @return
     */
    public static String getLastChange(String date) {
        if (date == null || date.equals(""))
            return null;
        String[] dateSplit = date.split(DateUtil.DATE_SPLIT);
        int year = new Integer(dateSplit[0]).intValue();
     ...
     * @return
     */
    public static List getTrendMonthDate(int dataYear, int dataMonth, int count, int freqId) {
        ...
        try {
            ...
            result = DateUtil.getDateList(startDate, endDate, new Integer(freqId));
    ...
    public static String getLastFreqDate(String date, Integer freq) {
        ...
        else if (freq.intValue() == DateUtil.FREQ_DAY) {
            return DateUtil.getLastDay(date);
        }
        else if (freq.intValue() == DateUtil.FREQ_WEEK) {
            return DateUtil.getLastWeek(date);
        }
        return null;
    }
     ...
     * @return
     */
    public static String getLastDateChange(String date) {
        if (date == null || date.equals(""))
            return null;
        String[] dateSplit = date.split(DateUtil.DATE_SPLIT);
        int year = new Integer(dateSplit[0]).intValue();
     ...
     * 
     * @param date
     * @param formatStr 格式如“yyyy-MM-dd”
     * @return
     */
    public static Date getDateByString(String date, String formatStr) {
        SimpleDateFormat format = new SimpleDateFormat(formatStr);
    ...
    public static java.sql.Timestamp stringToTime(String dateString) throws java.text.ParseException {

        DateFormat dateFormat;
        dateFormat = new SimpleDateFormat("yyyy-MM", Locale.ENGLISH);// 设定格式
     ...
     * @return
     */
    public static String getFreq(String date) {
        if (StringUtil.isEmpty(date) || date.length() != 10 || date.indexOf("-") == -1) {
            return "";
        }

        // 日频度
        String freq = String.valueOf(FREQ_DAY);
        // 如是周日增加周频度
        if (DateUtil.isSunday(date)) {
            freq += "," + FREQ_WEEK;
        }
        // 如是月底增加月频度
        if (DateUtil.isMonthDay(date)) {
            freq += "," + FREQ_MONTH;
        }
        // 如是季底增加季频度
        if (date.indexOf("03-31") > 0 || date.indexOf("09-30") > 0) {
            freq += "," + FREQ_SEASON;
        }
        // 如是半年底增加季频度、半年频度
        else if (date.indexOf("06-30") > 0) {
            freq += "," + FREQ_SEASON + "," + FREQ_HALFYEAR;
        }
        // 如是年底增加季频度、半年频度、年频度
        else if (date.indexOf("12-31") > 0) {
            freq += "," + FREQ_SEASON + "," + FREQ_HALFYEAR + "," + FREQ_YEAR;
        }

        return freq;
    }

    // 将timestamp格式的string字符串转换成yyyy-MM-dd.HH.mm.ss格式的字符串
    public static String timeStampToString(String date) {
    ...
    public static String timeToChage(String date) {
        DateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        try {
            Date d = format.parse(date);
    ...
    public static void main(String[] args) throws ParseException {
        ...
        Date date = sdf.parse("2013-08-31");
        System.out.println(sdf.format(DateUtil.countDateForLongValue(date, 5 - 1)));
        System.out.println(DateUtil.getToday("HH"));
    }
}
